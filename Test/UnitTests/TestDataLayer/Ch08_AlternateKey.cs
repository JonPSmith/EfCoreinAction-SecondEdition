        ...
        public void TestPersonNullAlternateKeyOk()
        {
            ...
            using (var context = new Chapter08DbContext(SqliteInMemory.CreateOptions<Chapter08DbContext>()))
            {
                ...
                ex.Message.ShouldEqual(
                    "Unable to track an entity of type 'Person' because alternate key property 'Email' is null. If the alternate key is not used in a relationship, then consider using a unique index instead. Unique indexes may contain nulls, while alternate keys may not.");
            }
        }
